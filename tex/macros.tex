...
\newcommand{\md}[1]{{\color{eoAttr}{\texttt{#1}}}}% modifiers: constant, sequential, opaque, injective
...
\newcommand{\ptn}[1]{{\color{eoConst}{\texttt{#1}}}}
\newcommand{\mcomment}[1]{{\color{eoComment}{\text{#1}}}}
\newcommand{\wrapp}[4]{{\color{#1}{#2}}{#4}{\color{#1}{#3}}}
\newcommand{\prn}[1]{\wrapp{eoPunct}{\lparen}{\rparen}{#1}}

\newcommand{\cvc}[1]{\textsc{cvc{#1}}}
\newcommand{\verit}{\textsc{veriT}}
\newcommand{\eunoia}{\textsc{Eunoia}}
\newcommand{\smtlib}{\textsc{smt-lib}}
\newcommand{\pnc}{{\color{eoPunct}}}
% ----- abstract syntax ----------
% plurality, binding
\newcommand{\plur}[2]{{#1}_{1}\ldots{#1}_{#2}}
\newcommand{\plurcomma}[2]{{#1}_{1}, \ldots, {#1}_{#2}}
% Grammar repetition markers — kept in the default foreground colour
% so the subscripts remain legible on print (cf. Reviewer B's comment).
\newcommand{\maybe}[1]{\langle #1 \rangle_{?}}
\newcommand{\many}[1]{\langle #1 \rangle_{+}}
\newcommand{\any}[1]{\langle #1 \rangle_{*}}
...
\newcommand{\eho}[2]{\eapp{\mtt{\_}}{#1\ #2}}% (_ t1 t2)
\newcommand{\earr}[1]{\epar{\ev{->}\ #1}}
\newcommand{\eprf}[1]{\epar{\mtt{Proof}\ #1}}
\newcommand{\ecase}[2]{\epar{#1\ #2}}
\newcommand{\eo}[1]{{\color{eoBuiltin}{\texttt{eo}}}{\color{eoPunct}{∷}}{\ev{#1}}}% builtins (Eunoia)
\newcommand{\lpo}[1]{{\color{eoBuiltin}{\texttt{eo}}}{\color{eoPunct}{.}}{\ev{#1}}}% builtins (LP)
...
\newcommand{\eoBuiltinFig}{
	\begin{array}[t]{r@{\ }l@{\ }l@{\quad}l}
		β & {⦂}
		  & \eo{eq}\ ∣\ \eo{requires}\ ∣\ \eo{quote}\ ∣\ \eo{var}\ ∣\ \ldots
		  & \mcomment{(core)}
		\\
		  & {∣}
...
\newcommand{\eoTermSyntax}{
	\begin{array}[t]{r@{\ }l@{\quad}l@{\qquad}r@{\ }l@{\quad}l}
		...
		ℓ & {⦂}\ \maybe{\ev{-}}n ∣ \maybe{\ev{-}}{n\ev{/}m}  ∣ \estr{\cdots}     & \mcomment{(literals)}        &
		ν & {⦂}\ \attr{implicit} \mid \attr{list}                                & \mcomment{(prm. attributes)}   \\[1mm]
	\end{array}
}
...
%
% --- macros for `lp.tex` ------------------------------
\newcommand{\esc}[1]{\wrapp{eoPunct}{⦃}{⦄}{#1}}
...
\newcommand{\lpConcreteSyntax}{
	\begin{array}[t]{r@{\ }c@{\ }l@{\quad}r}
		c & {⦂}
		  & \maybe{m}\, \kw{symbol}\ s \ \any{ρ} \,{\pnc : t} \ \maybe{≔ t'}\pnc;
		  & \mcomment{(commands)}
		\\
		  & {∣}
		  & \kw{rule}\ r\ \any{\kw{with}\ r'}\pnc;
		\\
		  & {∣}
		  & \kw{assert}\ ⊢\ t \pnc: t'\pnc;
		\\
		  & {∣}
		  & \kw{require}\,\kw{open}\,\many{μ}\pnc;
	\end{array}}
