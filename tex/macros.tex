...
\newcommand{\eo}[1]{\mtt{eo::#1}}
\newcommand{\etype}{\mtt{Type}}
\newcommand{\eoreqs}[3]{\eapp{\eo{requires}}{#1\ #2\ #3}}
\newcommand{\equote}[1]{\eapp{\eo{quote}}{#1}}

\newcommand{\attr}[1]{\mtt{:#1}}
...
\newcommand{\ch}[1]{\mtt{:chainable}\ {#1}}
\newcommand{\pw}[1]{\mtt{:pairwise}\ {#1}}
\newcommand{\bd}[1]{\mtt{:binder}\ {#1}}
\newcommand{\al}[1]{\mtt{:arg-list}\ {#1}}
...
\newcommand{\eoBuiltinFig}{
	\begin{array}[t]{r@{\ }l@{\quad}l}
		β & \begin{array}[t]{c@{\ }l}
			    {⦂} & \eo{ite}\ ∣\ \eo{eq}\ ∣\ \eo{requires}\ ∣\ \eo{typeof} \\
			    {∣} & \eo{var}\ ∣\ \eo{is\_ok}\ ∣\ \eo{is\_eq}               \\
			    {∣} & \eo{and}\ ∣\ \eo{or}\ ∣\ \eo{not}\ ∣\ \eo{xor}         \\
			    {∣} & \eo{add}\ ∣\ \eo{mul}\ ∣\ \eo{neg}\ ∣\ \eo{is\_neg}    \\
			    {∣} & \eo{nil}\ ∣\ \eo{cons}\ ∣\ \eo{list\_concat}           \\
			    {∣} & \eo{list\_len}\ ∣\ \eo{list\_nth}\ ∣\ \eo{list\_find}  \\
			    {∣} & \ldots
		    \end{array}
		  & \mcomment{(built-in operators)}
	\end{array}
}
...
\newcommand{\eoAttrSyntax}{
	\begin{array}[t]{r@{\ }c@{\ }l@{\quad}l}
		ν & {⦂} & \mtt{:implicit} \mid \mtt{:list}               & \mcomment{(prm. attributes)}   \\
		α & {⦂} & \mtt{:right-assoc} ∣ \mtt{:right-assoc-nil}\ t & \mcomment{(const. attributes)} \\
		  & {∣} & \mtt{:left-assoc} ∣ \mtt{:left-assoc-nil}\ t                                    \\
		  & {∣} & \mtt{:chainable}\ s ∣ \mtt{:pairwise}\ s                                        \\
		  & {∣} & \mtt{:binder}\ s ∣ \mtt{:arg-list}\ s
	\end{array}
}
...
\newcommand{\eoCommandSyntax}{
	\begin{array}[t]{r@{\ }l@{\quad}l}
		δ & \begin{array}[t]{c@{\ }l}
			    ...
			    \\ & \quad \maybe{\mtt{:assumption}\ t_{\text{assm}}}\
			    \\ & \quad \maybe{
			    \mtt{:premises}\ \epar{\vec{t}_{\text{prem}}}
			    \mid
			    \mtt{:premise-list}\ t_{\text{prem}}\ t_{\text{op}}
			    }\
			    \\ & \quad \maybe{\mtt{:args}\ \epar{\vec{t}_{\text{args}}}}\
			    \\ & \quad \maybe{\mtt{:requires}\ \epar{\vec{c}}}\
