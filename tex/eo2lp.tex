...
\author{Ciarán Dunne\inst{1}, Guillaume Burel\inst{2,3}}
...
\begin{abstract}
	\noindent
	Eunoia is a logical framework designed for specifying
	the proofs and proof systems of SMT solvers.
	%
	We present a translation from Eunoia to the
	\emph{$λΠ$-calculus modulo rewriting} as
	implemented by the $\lambdapi$ proof assistant.
	%
	The translation is realized by our tool
	\texttt{eo2lp}, which we use for
	%
	(a) translating a large fragment of \texttt{cvc5}'s
	\textit{co-operating proof calculus} (CPC) to $\lambdapi$,
	and
	%
	(b) translating a curated selection of proofs produced
	\texttt{cvc5} within this fragement.
\end{abstract}
